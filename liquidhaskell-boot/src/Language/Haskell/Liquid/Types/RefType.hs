...
import           Language.Fixpoint.Types hiding (DataDecl (..), QuotDecl (..), DataCtor (..), QuotCtor (..), panic, shiftVV, Predicate, isNumeric)
...
instance FreeVar RTyCon RTyVar where
  ...
  freeVars (RQTyCon _ _ _ vs _) = vs
...
instance Hashable RTyCon where
  ...
  hashWithSalt i (RQTyCon n _ _ _ _) = hashWithSalt i n
...
addNumSizeFun rt@(RTyCon _ _ inf)
  = rt {rtc_info = inf {sizeFunction = Just IdSizeFun } }
addNumSizeFun qt = qt
...
instance SubsTy RTyVar RSort RQuotient where
  subt z (RQuotient name tc vars l r) =
    RQuotient name tc (subt z <$> vars) l r

instance SubsTy RTyVar RSort RTyCon where
   subt z (RTyCon tc pvs inf) = RTyCon tc ps' inf
     where
       ps'  = subt z <$> pvs
   subt z (RQTyCon qc ty quots vars vs)
    = RQTyCon qc (subt z ty) (map (subt z) quots) vars vs
...
rTypeSort tce = typeSort tce . toType True -- . eraseQuotientTyCons' . void
...
instance PPrint QuotDecl where
  pprintTidy k qd =
    let
      prefix = "data" <+> pprint (qdName qd) <+> pprint (qdTyVars qd)
    in prefix <+> "=" $+$ nest 4 (vcat $ [ "\\" <+> pprintTidy k c | c <- qdQCons qd ])

instance PPrint QuotCtor where
  pprintTidy k (QuotCtor c as bs l r)
    =   pprintTidy k c <+> dcolon <+> ppVars k as <+> ppBinds bs
    <+> pprintTidy k l <+> pprintTidy k r
    where
      ppBinds [] = empty
      ppBinds ((v, t):vts)
        = pprintTidy k v <+> ":" <+> pprintTidy k t <+> "->" <+> ppBinds vts
  ...
  where
    ...
    go p (RApp c ts _ _)
      = case c of
          ...
          RQTyCon {rqtyVariances = vs} -> mconcat (zipWith go (map (getPosition p) vs) ts)
