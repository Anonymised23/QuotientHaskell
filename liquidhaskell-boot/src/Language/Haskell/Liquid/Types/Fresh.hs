...
instance (Freshable m Integer, Monad m, Applicative m) => Freshable m F.Expr where
  fresh :: (Freshable m Integer, Monad m, Applicative m) => m F.Expr
  fresh  = kv <$> fresh
    where
      kv = (`F.PKVar` mempty) . F.intKvar

instance (Freshable m Integer, Monad m, Applicative m) => Freshable m F.QPattern where
  fresh :: (Freshable m Integer, Monad m, Applicative m) => m F.QPattern
  fresh = fv <$> fresh
    where
      fv :: Integer -> F.QPattern
      fv = F.QPVar . F.intSymbol "k_"
...
{-
refreshRQuotient :: Freshable m Integer => Bool -> RQuotient -> m RQuotient
refreshRQuotient allowTC rq = do
  left  <- refresh allowTC $ rqtLeft rq
  right <- refresh allowTC $ rqtRight rq
  vars  <- traverse (refreshRefType allowTC) $ rqtVars rq
  return rq
    { rqtLeft  = left
    , rqtRight = right
    , rqtVars  = vars
...
{-
refreshRefType allowTC (RApp (RQTyCon nm ut qs tvs vs) ts rs r)
  = RApp <$> tycon <*> mapM (refresh allowTC) ts <*> mapM (refreshRef allowTC) rs <*> refresh allowTC r
  where
    tycon = do
      utype <- refreshRefType allowTC ut
      nqs   <- traverse (refreshRQuotient allowTC) qs
      return $ RQTyCon nm utype nqs tvs vs-}
...
{-
refreshVVQuot :: Freshable m Integer => RQuotient -> m RQuotient
refreshVVQuot (RQuotient nm tc vs l r) = do
  vs' <- traverse refreshVV vs
  return $ RQuotient nm tc vs' l r-}
