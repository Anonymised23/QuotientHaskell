...
data QuotientSpec = SpQuotient
  { qsName      :: !(F.Located F.Symbol)
  , qsUType     :: !SpecType
  , qsQuots     :: ![F.Symbol]
  , qsTyVars    :: ![RTyVar]
  } deriving Show
...
data GhcSpecQuots = SpQuots
  { gsQuotTyCons  :: !(M.HashMap F.Symbol QuotientSpec)
  , gsQuotients   :: !(M.HashMap F.Symbol RQuotient)
  }
...
instance Monoid GhcSpecQuots where
  mempty = SpQuots { gsQuotTyCons = mempty, gsQuotients = mempty }
...
data Spec ty bndr  = Spec
  ...
  , respSigs   :: ![RespectsSig]                  -- ^ Explicit quotient respectfulness signatures
...
data LiftedSpec = LiftedSpec
  ...
  , liftedQuotDecls  :: HashSet QuotDecl
    -- ^ Quotiented data definitions
