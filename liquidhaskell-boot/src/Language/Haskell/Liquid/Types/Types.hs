...
module Language.Haskell.Liquid.Types.Types (
  ...
  -- Data type for representing explicit quotient respectability signature
  , RespectsSig (..)
...
data RQuotient = RQuotient
  { rqtName   :: !(F.Located F.Symbol)
  , rqtTyCon  :: !(F.Located F.Symbol)
  , rqtVars   :: !(M.HashMap F.Symbol SpecType)
  , rqtLeft   :: !F.QPattern
  , rqtRight  :: !F.Expr
  }
  deriving (Generic, Data, Typeable)
...
data RTyCon
  ...
  | RQTyCon                        -- ^ Quotient Type Constructor
      { rqtyName      :: !(F.Located F.Symbol)
      , rqtyUType     :: !SpecType
      , rqtyQuots     :: ![F.Symbol]
      , rqtyTyVars    :: ![RTyVar]
      , rqtyVariances :: ![Variance]
      }
  deriving (Generic, Data, Typeable)

instance F.Symbolic RQuotient where
  symbol = F.symbol . rqtName

instance F.Symbolic RTyCon where
  symbol (RTyCon  tc _ _)     = F.symbol tc
  symbol (RQTyCon qc _ _ _ _) = F.symbol qc
...
rTyConPVs (RTyCon _ ps _) = ps
rTyConPVs _               = []

rTyConPropVs :: RTyCon -> [PVar RSort]
rTyConPropVs (RTyCon _ ps _) = filter isPropPV ps
rTyConPropVs _               = []
...
tyConPred :: (TyCon -> Bool) -> RTyCon -> Bool
tyConPred p (RTyCon c _ _) = p c
tyConPred _ _              = False

instance TyConable RTyCon where
  isFun      = tyConPred isFunTyCon
  isList     = tyConPred (listTyCon ==)
  isTuple    = tyConPred Ghc.isTupleTyCon
  isClass    = tyConPred isClass -- isClassRTyCon
  isEqual    = tyConPred isEqual
  ppTycon    = F.toFix

  isNumCls = tyConPred $ maybe False (isClassOrSubClass isNumericClass) . tyConClass_maybe

  isFracCls = tyConPred $ maybe False (isClassOrSubClass isFractionalClass) . tyConClass_maybe

  isOrdCls  = tyConPred $ maybe False isOrdClass . tyConClass_maybe
  isEqCls   = tyConPred isEqCls
...
instance Eq RTyCon where
  (==) :: RTyCon -> RTyCon -> Bool
  (RTyCon x _ _)      == (RTyCon y _ _)      = x == y
  (RQTyCon x _ _ _ _) == (RQTyCon y _ _ _ _) = x == y
  _                   == _                   = False
...
instance F.Fixpoint RTyCon where
  ...
  toFix (RQTyCon c _ _ _ _) = text $ show $ F.val c -- (needs correct construction)
...
instance F.PPrint RTyCon where
  pprintTidy k (RTyCon rtc_tc rtc_pvars _)
    | ppDebug ppEnv = F.pprintTidy k tc  <-> angleBrackets (F.pprintTidy k rtc_pvars)
    | otherwise     = text $ showPpr rtc_tc
    where
      tc            = F.symbol rtc_tc
  pprintTidy k (RQTyCon qtc _ _ _ _)
    | ppDebug ppEnv = F.pprintTidy k $ F.val qtc
    | otherwise     = F.pprintTidy k $ F.val qtc
...
data QuotDecl = QuotDecl
  { qdName   :: !F.LocSymbol -- ^ Quotient type name
  , qdTyVars :: [Symbol]     -- ^ Tyvar Parameters
  , qdUType  :: BareType     -- ^ Underlying type
  , qdQCons  :: [QuotCtor]   -- ^ Quotient Constructors
  , qdSrcPos :: !F.SourcePos -- ^ Source Position
...
instance Eq QuotDecl where
  q1 == q2 = qdName q1 == qdName q2

instance Ord QuotDecl where
  compare q1 q2 = compare (qdName q1) (qdName q2)
...
instance F.Loc QuotDecl where
  srcSpan = srcSpanFSrcSpan . sourcePosSrcSpan . qdSrcPos
...
instance Show QuotDecl where
  show qd = printf "QuotDecl: data = %s, tyvars = %s" -- [at: %s]"
              (show $ qdName   qd)
              (show $ qdTyVars qd)
