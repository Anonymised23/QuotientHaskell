...
import           Language.Fixpoint.Types                    hiding (dcFields, DataDecl, QuotDecl, Error, panic)
...
makeGhcSpec0 cfg src lmap mspecsNoCls = do
  ...
  -- _ <- error (show $ Bare.tcSelMeasures tycEnv0)
  pure (diags, SP
    { _gsConfig = cfg
...
makeSpecQuots :: Bare.Env -> [(ModName, Ms.BareSpec)] -> Bare.Lookup GhcSpecQuots
makeSpecQuots env specs
  ...
  where
    ...
    addQuotDecl mname qdecl spec
      = case Bare.maybeResolveSym env mname "quotient-spec" (qdName qdecl) of
          Just (qt :: Bare.QuotTy, qs) -> do
            tcs   <- toQuotientSpec mname qt
            quots <- traverse (toRQuotient mname) qs
            return spec
              { gsQuotTyCons
                  = M.insert (F.val $ qsName tcs) tcs (gsQuotTyCons spec)
              , gsQuotients
                  =  gsQuotients spec
                  <> M.fromList [ (F.val $ rqtName q, q) | q <- quots ]
              }
          Nothing                  -> return spec

    toQuotientSpec :: ModName -> Bare.QuotTy -> Bare.Lookup QuotientSpec
    toQuotientSpec mname qt = do
      let pos = F.loc $ Bare.qtyName qt
      utype <- Bare.ofBareTypeE env mname pos Nothing $ Bare.qtyUType qt
      return SpQuotient
        { qsName   = Bare.qualifyTop env mname pos $ Bare.qtyName qt
        , qsUType  = Bare.qualifyTop env mname pos utype
        , qsQuots  = Bare.qualifyTop env mname pos <$> Bare.qtyQuots qt
        , qsTyVars = map symbolRTyVar $ Bare.qtyTyVars qt
        }

    toRQuotient :: ModName -> Bare.Quotient -> Bare.Lookup RQuotient
    toRQuotient mname q = do
      let pos = F.loc $ Bare.qtName q
      vars <- traverse (Bare.ofBareTypeE env mname pos Nothing) (Bare.qtVars q)
      return
        RQuotient
          { rqtName  = Bare.qualifyTop env mname pos $ Bare.qtName q
          , rqtTyCon = Bare.qualifyTop env mname pos $ Bare.qtTyCon q
          , rqtVars  = Bare.qualifyTop env mname pos <$> vars
          , rqtLeft  = Bare.qualifyTop env mname pos $ Bare.qtLeft q
          , rqtRight = Bare.qualifyTop env mname pos $ Bare.qtRight q
          }
