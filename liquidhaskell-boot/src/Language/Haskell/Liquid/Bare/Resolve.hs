...
makeQuotMaps = foldr updateQuotMap mempty
  where
    updateQuotMap :: (ModName, BareSpec) -> M.HashMap ModName QuotMap -> M.HashMap ModName QuotMap
    updateQuotMap (modName, spec)
      = M.insertWith (<>) modName (foldr addQuotDecl mempty (quotDecls spec))

    -- addQuotDecls :: [QuotDecl] -> QuotMap -> QuotMap
    -- addQuotDecls decls qm = foldr addQuotDecl qm decls

addQuotDecl :: QuotDecl -> QuotMap -> QuotMap
addQuotDecl qd qm
  = QuotMap
      { quotTyMap = M.insert (F.val $ qdName qd) quotTy $ quotTyMap qm
      , quotMap   = foldr (\(k, v) m -> M.insert (F.val k) v m) (quotMap qm) quotients
      }
  where
    quotients = map (makeQuotient $ qdName qd) $ qdQCons qd
    ...
    quotTy
      = QuotTy
          ...
          , qtyTyVars = qdTyVars qd
          }

makeQuotient :: F.Located F.Symbol -> QuotCtor -> (F.Located F.Symbol, Quotient)
...
instance Qualify RTyCon where
  qualify env name l bs rtc@(RTyCon _ _ inf)    = rtc { rtc_info = qualify env name l bs inf }
  qualify env name l bs qtc@(RQTyCon n utype qs _ _)
    = qtc
        { rqtyName  = qualify env name l bs n
        , rqtyUType = qualify env name l bs utype
        , rqtyQuots = qualify env name l bs qs
        }

instance Qualify RQuotient where
  qualify env name l bs qt
    = qt
        { rqtName  = qualify env name l bs (rqtName qt)
        , rqtTyCon = qualify env name l bs (rqtTyCon qt)
        , rqtVars  = qualify env name l bs <$> rqtVars qt
        , rqtLeft  = qualify env name l bs (rqtLeft qt)
        , rqtRight = qualify env name l bs (rqtRight qt)
        }
...
instance Qualify F.QPattern where
  qualify env name l bs (F.QPCons c ps)
    = F.QPCons (qualify env name l bs c) (qualify env name l bs ps)
  {-qualify env name l bs (F.QPRec c fs)
    = F.QPRec (qualify env name l bs c) (qualify env name l bs fs)-}
  {-qualify env name l bs (F.QPTup ps)
    = F.QPTup (qualify env name l bs ps)-}
  qualify _ _ _ _ (F.QPLit l) = F.QPLit l
  qualify _ _ _ _ (F.QPVar v) = F.QPVar v


instance Qualify QuotCtor where
  qualify env name l bs qc = qc
    { qcName  = qualify env name l bs (qcName qc)
    , qcBinds = map (\(s, t) -> (s, qualify env name l bs t)) (qcBinds qc)
    , qcLeft  = qualify env name l bs (qcLeft qc)
    , qcRight = qualify env name l bs (qcRight qc)
    }

instance Qualify QuotDecl where
  qualify env name l bs q = q
    { qdName  = qualify env name l bs (qdName q)
    , qdUType = qualify env name l bs (qdUType q)
    , qdQCons = qualify env name l bs (qdQCons q)
    }
...
instance ResolveSym (QuotTy, [Quotient]) where
  resolveLocSym = resolveQuotTyWith "quotient constructor"
...
resolveQuotTyWith :: PJ.Doc -> Env -> ModName -> String -> LocSymbol -> Lookup (QuotTy, [Quotient])
resolveQuotTyWith kind env name str lx
  = case qTy of
      Nothing -> Left [errResolve kind str lx]
      Just qt -> Right qt
  where
    qTy :: Maybe (QuotTy, [Quotient])
    qTy = do
...
ofBRType :: forall r. (Expandable r) => Env -> ModName -> ([F.Symbol] -> r -> r) -> F.SourcePos -> BRType r
         -> Lookup (RRType r)
ofBRType env name f l = go []
  where
    ...
    go :: [F.Symbol] -> RType BTyCon BTyVar r -> Lookup (RType RTyCon RTyVar r)
    ...
    goRApp 
      ...
      -> [Ref BSort (RType BTyCon BTyVar r)]
      -> r
      -> Either [Error] (RType RTyCon RTyVar r)
    goRApp bs tc ts rs r
      = case mlc' of
          Left _  -> do
            (qt, _ :: [Quotient]) <- resolveLocSym env name "" lc
            as <- mapM (go bs) ts
            bareQCApp env name l r qt as
          Right lc' -> bareTCApp <$> goReft bs r <*> Right lc' <*> mapM (goRef bs) rs <*> mapM (go bs) ts
      where
        mlc'  = F.atLoc lc <$> matchTyCon env name lc (length ts)
        lc    = btc_tc tc
  ...
  where
    ...
    go p (RApp c ts _ _)          mp
      = case c of
          ...
          RQTyCon {rqtyUType = utype, rqtyTyVars = tvs, rqtyVariances = vs}
...
{-
toRQuotient :: F.Located F.Symbol -> Quotient -> RQuotient
toRQuotient tc qt =
  RQuotient
    { rqtName  = qtName qt
    , rqtTyCon = tc
    , rqtVars  = ofBareType env modname (F.loc $ qtName qt) Nothing <$> qtVars qt
    , rqtLeft  = qtLeft qt
    , rqtRight = qtRight qt
    }-}
...
bareQCApp env modname pos r (QuotTy name ty quots vs) ts
  ...
  where
    ...
    tycon =
      RQTyCon
        { rqtyName      = name
        , rqtyUType     = ofBareType env modname pos Nothing ty
        , rqtyQuots     = quots
        , rqtyTyVars    = ftvs
        , rqtyVariances = [ Mb.fromMaybe Invariant (M.lookup v variances) | v <- ftvs ]
        }
