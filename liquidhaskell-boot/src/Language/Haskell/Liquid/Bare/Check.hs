...
checkBareSpec _ sp
  ...
  where
    ...
    nonRecQs  = foldMap checkNonRecursiveQuotient $ Ms.quotDecls sp
...
quotDeclVars = map qcName . qdQCons
...
checkNonRecursiveQuotient :: QuotDecl -> Diagnostics
checkNonRecursiveQuotient qdecl
  = case containsTyCon (F.val $ qdName qdecl) (qdUType qdecl) of
      Just pos ->
          mkDiagnostics []
            [ ErrRecQuot
                (GM.sourcePosSrcSpan $ F.loc $ qdName qdecl)
                (pprint $ qdName qdecl)
                (GM.sourcePosSrcSpan pos)
                (pprint $ qdUType qdecl)
            ]
...
checkQuotients allowHO bsc _ emb tcEnv senv quots
  ...
  where
    quotientMap :: [F.Symbol] -> [RQuotient]
    quotientMap syms = mapMaybe (`M.lookup` gsQuotients quots) syms

    checkQuotientTy :: QuotientSpec -> Diagnostics
    checkQuotientTy spec
      =  checkTy allowHO bsc err emb tcEnv senv (F.Loc loc loc $ qsUType spec)
      <> freeTyVarDiagnostics
      <> foldMap (checkQuotient (qsUType spec)) (quotientMap $ qsQuots spec)
      where
        loc = F.loc $ qsName spec

        err = ErrQuotType (GM.sourcePosSrcSpan loc) (pprint $ qsName spec)

        unboundTyVars :: [RTyVar]
        unboundTyVars = map ty_var_value (freeTyVars $ qsUType spec) L.\\ qsTyVars spec
    ...
    checkQuotient :: SpecType -> RQuotient -> Diagnostics
    checkQuotient ty qt = lCheck <> rCheck
      where
        ...
        lCheck
          ...
          $ checkSortFull (F.srcSpan $ rqtName qt) γ (rTypeSort emb ty) (rqtLeft qt)

        rCheck
          = checkETy "right-hand side"
          $ checkSortFull (F.srcSpan $ rqtName qt) γ (rTypeSort emb ty) (rqtRight qt)
        ...
        err doc = ErrQuotWF
          { pos   = GM.sourcePosSrcSpan $ F.loc $ rqtName qt
          , qname = pprint $ rqtName qt
          , dc    = doc
          }
...
{-
    ...
    eraseQuotTyCon (RFun b inf i o _)
      = RFun b inf (eraseQuotTyCon i) (eraseQuotTyCon o) ()
    eraseQuotTyCon (RAllT tvb ty _)
      = RAllT (eraseQuotTyCon <$> tvb) (eraseQuotTyCon ty) ()
    eraseQuotTyCon (RAllP pvb ty)
    ...
    eraseQuotTyCon (RAllE s aa ty)
      = RAllE s (eraseQuotTyCon aa) (eraseQuotTyCon ty)
    eraseQuotTyCon (REx s exa ty)
      = REx s (eraseQuotTyCon exa) (eraseQuotTyCon ty)
    eraseQuotTyCon (RExprArg e) = RExprArg e
    eraseQuotTyCon (RAppTy a r _)
      = RAppTy (eraseQuotTyCon a) (eraseQuotTyCon r) ()
...
errTypeMismatch x t = ErrMismatch lqSp (pprint x) (text "Checked") d1 d2 Nothing hsSp
  where
    d1              = pprint $ varType x -- text $ showRType ((ofType $ varType x) :: RType RTyCon RTyVar ())
    d2              = pprint $ toType False $ val t -- text $ showRType $ toRSort $ F.val t
    lqSp            = GM.fSrcSpan t
    hsSp            = getSrcSpan x
...
checkTcArity tycon givenArity
  = case tycon of
      ...
      RQTyCon { rqtyName = name, rqtyTyVars = vars }
        | expectedArity < givenArity ->
            Just $ text "Quotient type constructor" <+> pprint name
              <+> text "expects a maximum" <+> pprint expectedArity
              <+> text "arguments but was given" <+> pprint givenArity
              <+> text "arguments"
        | otherwise                  -> Nothing
        where
          expectedArity = length vars
...
checkReft sp env emb (Just t) _ = (\z -> dr $+$ z) <$> checkSortedReftFull sp env r
  where
    ...
    dr                          = text "Sort Error in Refinement:" <+> pprint r <+> text ", Env: " <+> pprint env
...
containsTyCon :: F.Symbol -> RType BTyCon a r -> Maybe F.SourcePos
containsTyCon s (RAllT rtvu t _) = firstJust p (containsTyCon s t)
  where
    p = case ty_var_info rtvu of
      RTVInfo { rtv_kind = k } -> containsTyCon s k
      _                        -> Nothing
containsTyCon s (RAllP pvu t) = firstJust p0 (firstJust p1 $ containsTyCon s t)
  where
    p0 = case ptype pvu of
      PVProp k -> containsTyCon s k
      _        -> Nothing

    p1 = firstJustMap (\(u, _, _) -> containsTyCon s u) (pargs pvu)
containsTyCon s (RApp tc ts ps _)
  | F.val (btc_tc tc) == s = Just $ F.loc $ btc_tc tc
  | otherwise = firstJust (firstJustMap (containsTyCon s) ts) (firstJustMap pp ps)
      where
        pp x
          = firstJust (containsTyCon s $ rf_body x)
                      (firstJusts $ map (containsTyCon s . snd) $ rf_args x)
containsTyCon s (RFun _ _ t1 t2 _)
  = firstJust (containsTyCon s t1) (containsTyCon s t2)
containsTyCon _ (RVar _ _)         = Nothing
containsTyCon s (RAllE _ t1 t2)
  = firstJust (containsTyCon s t1) (containsTyCon s t2)
containsTyCon s (REx _ t1 t2)
  = firstJust (containsTyCon s t1) (containsTyCon s t2)
containsTyCon s (RAppTy t1 t2 _)
  = firstJust (containsTyCon s t1) (containsTyCon s t2)
containsTyCon s (RRTy ts _ _ t)
  = firstJust (firstJustMap (containsTyCon s . snd) ts) (containsTyCon s t)
containsTyCon _ (RExprArg _)       = Nothing
containsTyCon _ (RHole _)          = Nothing
