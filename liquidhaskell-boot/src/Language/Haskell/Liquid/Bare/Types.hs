...
data Quotient = Quotient
  { qtName   :: !(F.Located F.Symbol)
  , qtTyCon  :: !(F.Located F.Symbol)
  , qtVars   :: !(M.HashMap F.Symbol BareType)
  , qtLeft   :: !F.QPattern
  , qtRight  :: !F.Expr
  }

data QuotTy = QuotTy
  { qtyName   :: !(F.Located F.Symbol)
  , qtyUType  :: !BareType
  , qtyQuots  :: ![F.Symbol]
  , qtyTyVars :: ![F.Symbol]
  }
...
data QuotMap = QuotMap
  { quotTyMap :: !(M.HashMap F.Symbol QuotTy)
  , quotMap   :: !(M.HashMap F.Symbol Quotient)
  }

instance Semigroup QuotMap where
  (<>) :: QuotMap -> QuotMap -> QuotMap
  q1 <> q2
    = QuotMap
