...
data CGEnv = CGE
  ...
  , cgQuotTyCons :: !(M.HashMap F.Symbol QuotientSpec)
  , cgQuotients  :: !(M.HashMap F.Symbol RQuotient)
  ...
  , binders :: !(S.HashSet F.Symbol)                 -- ^ Variables bound by lambdas/case expressions (free in open terms)
  ...
  , isCheckDataConApp :: !Bool                       -- ^ Whether the current check is for application of a data constructor (for quotient types)
...
data WfC      = WfC  !CGEnv !SpecType deriving Show
              -- deriving (Data, Typeable)
...
instance NFData CGEnv where
  rnf (CGE x1 _ x3 _ x4 x5 _ _ x55 x6 x7 x8 x9 _ _ _ x10 _ _ _ _ _ _ _ _ _ _ _ _)
