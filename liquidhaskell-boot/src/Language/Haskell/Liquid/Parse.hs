...
import           Language.Fixpoint.Types   as F hiding (panic, SVar, DDecl, DataDecl, QuotDecl (..), DataCtor (..), QuotCtor (..), Error, R, Predicate)
...
data Pspec ty ctor
  ...
  | QAsrt   RespectsSig                                   -- ^ Explicit quotient respectfulness signature
  ...
  | QDecl   QuotDecl                                      -- ^ quotiented 'data' declaration
...
quotPatternP :: Parser F.QPattern
quotPatternP
  =   tupPat
  <|> (upperIdP >>= consPat)
  <|> (QPLit <$> constantP)
  ...
  where
    ...
    tupPat = do
      (first, rest) <- parens ((,) <$> quotPatternP <* comma <*> sepBy1 quotPatternP comma) -- at least two components necessary
      let cons = symbol $ "(" ++ replicate (length rest) ',' ++ ")" -- stored in prefix form
      return $ QPCons cons (first : rest)

    consPat x = {-QPRec x <$> recPat <|> (-} QPCons x <$> many argPat {-)-}

    {-recPat =
      braces (sepBy ((,) <$> lowerIdP <* reservedOp "=" <*> quotPatternP) (reservedOp ","))-}
    ...
    []
      | vanilla -> do
          ...
          <|> ( do
                  ...
                  return $ QDecl $ QuotDecl x as utype qtors pos
              )
...
{-
dataCtorsP :: [Symbol] -> Parser (Maybe BareType, [DataCtor], [QuotCtor])
dataCtorsP as = do
  (pTy, dcs, qcs) <- (reservedOp "="     >> (   (Nothing,,)
                                            <$> sepBy (dataConP as) (reservedOp "|")
                                            <*> many (reservedOp "|/" *> quotConP as)
                                            ))
                 <|> (reserved   "where" >> ( do
                                              (dc, qc) <- dataAndQuotBlockP as
                                              return (Nothing, dc, qc)
                                            ))
                 <|>                        ( do
                                               dp       <- dataPropTyP
                                               (dc, qc) <- dataAndQuotBlockP as
  ...
  return (pTy, Misc.sortOn (val . dcName) dcs, qcs)
-}
