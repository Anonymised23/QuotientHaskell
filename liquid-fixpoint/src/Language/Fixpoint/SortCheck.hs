...
toExpr (QPCons c ps) = L.foldl' EApp (EVar c) (toExpr <$> ps)
...
checkQPattern f p@(QPCons _ _)  = checkExpr f (toExpr p)
...
{-checkFunSortWith :: Sort -> [Sort] -> CheckM (Sort, TVSubst)
checkFunSortWith s = foldrM checkInput (s, emptySubst)
  where
    checkInput :: Sort -> (Sort, TVSubst) -> CheckM (Sort, TVSubst)
    checkInput f (inp, subst) = do
      (i, o, s) <- checkFunSort f
      case unifySorts i inp of
        Nothing -> throwErrorAt
          ( err dummySpan
              ( "sortCheck: applied invalid type to "
              <+> pprint f   <+> " expected "
              <+> pprint inp <+> " but found "
              <+> pprint i   <+> "."
              )
          )
        Just ns -> return (o, subst <> s <> ns)-}
