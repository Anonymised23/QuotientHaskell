...
data QPattern
  = QPLit !Constant
  | QPVar !Symbol
  | QPCons !Symbol ![QPattern]
  -- | QPRec !Symbol ![(Symbol, QPattern)]
  -- | QPTup ![QPattern]
...
instance PPrint QPattern where
  ...
  pprintPrec z k (QPCons c ps)
    = parensIf (z > za) $ pprintPrec za k c <+> pprintPrec (za+1) k ps
    where za = 8
  {-pprintPrec z k (QPRec c fs)
    = parensIf (z > za) $ pprintPrec za k c <+> "{" <+> pprintPrec za k (hcat $ ppFields fs) <+> "}"
    where
      za = 8

      ppFields = punctuate ", " . map (\(v, p) -> pprintTidy k v <+> "=" <+> pprintTidy k p) -}
  -- pprintPrec _ k (QPTup ps) = "(" <+> hcat (punctuate ", " $ map (pprintTidy k) ps) <+> ")"
