...
module Language.Fixpoint.Types.Sorts (
  ...
  , isNumeric, isReal, isString, isPolyInst, isChar
...
data QuotBind = QBind
  { qbName :: !LocSymbol -- ^ Quotient bind
  , qbSort :: !Sort      -- ^ Quotient sort
  } deriving (Eq, Ord, Show, Data, Typeable, Generic)
...
data QuotCtor e = QCtor
  { qcName  :: !LocSymbol -- ^ Quotient constructor name
  , qcBinds :: ![QuotBind]
  , qcLeft  :: e
  , qcRight :: e
  } deriving (Eq, Ord, Show, Data, Typeable, Generic)

data QuotDecl e = QDecl
  { qdTyCon  :: !FTycon      -- ^ Name of defined quotient type
  , qdVars   :: !Int         -- ^ Number of type variables
  , qdUSort  :: !Sort        -- ^ Underlying sort
  , qdQtors  :: [QuotCtor e] -- ^ Quotient constructors
  } deriving (Eq, Ord, Show, Data, Typeable, Generic)
...
instance Loc (QuotDecl e) where
    srcSpan (QDecl ty _ _ _) = srcSpan ty

instance Symbolic DataDecl where
  symbol = symbol . ddTyCon

instance Symbolic (QuotDecl e) where
  symbol = symbol . qdTyCon
...
muSortQ :: [QuotDecl e] -> [QuotDecl e]
muSortQ qds = mapSortQuotDecl tx <$> qds
  where
    selfs = [(fTyconSelfSort c n, fTyconSort c) | QDecl c n _ _ <- qds]
    tx t  = fromMaybe t $ L.lookup t selfs

    mapSortQuotDecl f  qd = qd { qdQtors = mapSortQuotCtor f  <$> qdQtors qd }
    mapSortQuotCtor f  qt = qt { qcBinds  = mapSortQuotBind f <$> qcBinds qt }
    mapSortQuotBind f  qb = qb { qbSort   = f $ qbSort qb } 
...
instance Fixpoint QuotBind where
  toFix (QBind x t) = toFix x <+> text ":" <+> toFix t

instance Fixpoint e => Fixpoint (QuotCtor e) where
  toFix (QCtor x bs l r)
    =   toFix x <+> text "::" <+> intersperse "->" (toFix <$> bs)
    <+> "->" <+> toFix l <+> text "==" <+> toFix r
...
instance Fixpoint e => Fixpoint (QuotDecl e) where
  toFix (QDecl qc n t qtors) = vcat (header:body ++ [footer])
    where
      header = toFix qc <+> toFix n <+> text "= " <+> toFix t <+> "["
      body   = [nest 2 (text "\\" <+> toFix qt) | qt <- qtors]
      footer = text "]"
...
instance PPrint QuotBind where
  pprintTidy _ = toFix

instance Fixpoint e => PPrint (QuotCtor e) where
  pprintTidy _ = toFix

instance PPrint DataDecl where
  pprintTidy _ = toFix

instance Fixpoint e => PPrint (QuotDecl e) where
  pprintTidy _ = toFix
...
instance S.Store QuotBind
instance S.Store e => S.Store (QuotCtor e)
instance S.Store DataDecl
instance S.Store e => S.Store (QuotDecl e)
...
instance NFData QuotBind
instance NFData e => NFData (QuotCtor e)
instance NFData DataDecl
instance NFData e => NFData (QuotDecl e)
instance NFData Sub
