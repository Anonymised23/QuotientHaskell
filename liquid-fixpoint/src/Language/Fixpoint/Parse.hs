...
quotBindP :: Parser QuotBind
quotBindP = QBind <$> locSymbolP <* colon <*> sortP
...
quotCtorP :: Parser (QuotCtor Expr)
quotCtorP = do
  x  <- locSymbolP <* dcolon
  bs <- sepBy quotBindP (reservedOp "->")
  l  <- reservedOp "->" *> exprP
  r  <- reservedOp "==" *> exprP
  return $ QCtor x bs l r

quotDeclP :: Parser (QuotDecl Expr)
quotDeclP = QDecl <$> fTyConP <*> intP <* reservedOp "="
                  <*> sortP
                  <*> brackets (many (reservedOp "\\" *> quotCtorP))
...
defP =  Srt   <$> (reserved "sort"         >> colon >> sortP)
    ...
    <|> (reserved "data" >> (Adt <$> try dataDeclP) <|> (Qdt <$> quotDeclP))
